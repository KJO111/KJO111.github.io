...
function decideWhichCellShouldBeAliveNextTurn(){
  ...
  for(var i = 0;i<arraySize;i++){
    ...
    //https://en.wikipedia.org/wiki/Conway%27s_Game_of_Life
     //if there's less than 2 neighbors the cell will die from underpopulation
    if(isAlive[i] && numberOfLivingNeighbors < 2){
    nextTurnIsAlive[i] = false
    }
    //if there's more than 3 neighbors the cell will die from overpopulation
    if(isAlive[i] && numberOfLivingNeighbors > 3){
    nextTurnIsAlive[i] = false
    }
    //if a cell has 3 neighbors it will either stay alive or a new cell will be born
